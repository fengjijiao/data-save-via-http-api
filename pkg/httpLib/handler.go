...
func enableCors(w * http.ResponseWriter, allowOriginUrl string) {
...
func setupResponse(w * http.ResponseWriter, r * http.Request) {
	if conf.Config.EnableCors {
		enableCors(w, conf.Config.CorsAllowOriginUrl)
	}
}
...
func RegisterHttpHandler(w http.ResponseWriter, r * http.Request) {
    setupResponse( &w, r)
	if !conf.Config.AllowRegister {
		json.NewEncoder(w).Encode(GenError(ErrorRegistrationNotAllowed))
		return
	}
    if err := r.ParseMultipartForm(parseMultipartFormMaxMemory);
    err != nil {
        json.NewEncoder(w).Encode(GenError(ErrorParsingData))
        return
    }
...
func PutValHttpHandler(w http.ResponseWriter, r * http.Request) {
    ...
    if err = sqliteLib.UpdateDataSetValue(dsid, newValStr);
    err != nil {
        json.NewEncoder(w).Encode(GenError(err.Error()))
        return
    }
    if err = sqliteLib.UpdateUpdatedTimestamp(did);
    err != nil {
        json.NewEncoder(w).Encode(GenError(err.Error()))
        return
    }
...
func DelValHttpHandler(w http.ResponseWriter, r * http.Request) {
    ...
    if err = sqliteLib.DelDataSource(did);
    err != nil {
        json.NewEncoder(w).Encode(GenError(err.Error()))
        return
    }
    if err = sqliteLib.DelDataSetViaDid(did);
    err != nil {
        json.NewEncoder(w).Encode(GenError(err.Error()))
        return
    }
